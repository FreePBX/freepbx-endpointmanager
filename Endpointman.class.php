...
//PERMITE TODO!!!!!!!!!!!!!!!!!!!
    ...
    public function epm_config_hardware_get_list_models($id_product = NULL, $show_all = true)
    {
        if (!is_numeric($id_product)) {
            throw new \Exception(_("ID Producto not is number") . " (" . $id_product . ")");
        }
        if ($show_all == true) {
            $sql = 'SELECT * FROM endpointman_model_list WHERE product_id = ' . $id_product;
        } else {
            $sql = 'SELECT * FROM endpointman_model_list WHERE hidden = 0 AND product_id = ' . $id_product;
        }
    ...
    public function epm_config_hardware_get_list_product($id_brand = NULL, $show_all = true)
    {
        if (!is_numeric($id_brand)) {
            throw new \Exception(_("ID Brand not is numbre") . " (" . $id_brand . ")");
        }
        if ($show_all == true) {
            $sql = 'SELECT * FROM endpointman_product_list WHERE brand = ' . $id_brand . ' ORDER BY long_name ASC';
        } else {
            $sql = 'SELECT * FROM endpointman_product_list WHERE hidden = 0 AND brand = ' . $id_brand . ' ORDER BY long_name ASC';
        }
        $result = sql($sql, 'getAll', DB_FETCHMODE_ASSOC);
        return $result;
    }
        ...
        if (file_exists($path_tmp_dir)) {
            if (is_dir($path_tmp_dir)) {
                ...
                foreach ($l_files as $archivo) {
                    if (in_array($archivo, $array_list_exception)) {
                        continue;
                    }
    ...
    public function epm_config_manager_hardware_get_list_all($check_for_updates = true)
    {
        ...
        foreach ($brand_list as $row) {
            ...
            if ($check_for_updates) {
                ...
                if (!isset($row_out[$i]['update'])) {
                    $row_out[$i]['update'] = "";
                }
                if (!isset($row_out[$i]['update_vers'])) {
                    $row_out[$i]['update_vers'] = "";
                }
            }
            if ($row['hidden'] == 1) {
            ...
            foreach ($product_list as $row2) {
                ...
                if ($check_for_updates) {
                    ...
                    if (!isset($row_out[$i]['products'][$j]['update_fw'])) {
                        $row_out[$i]['products'][$j]['update_fw'] = 0;
                    }
                    if (!isset($row_out[$i]['products'][$j]['update_vers_fw'])) {
                        $row_out[$i]['products'][$j]['update_vers_fw'] = "";
                    }
                }
    ...
    function update_check($echotxt = false)
    {
        ...
        if (!$this->configmod->get('use_repo')) {
            ...
            if (!$master_result) {
                ...
                if ($echotxt) {
                    echo format_txt(_("Not able to connect to repository. Using local master file instead."), "error");
                }
            }
            ...
            if ($contents != '1') {
                if (($data == "") OR ($data <= $endpoint_last_mod)) {
                    if ((!$master_result) OR (!$this->system->download_file($this->UPDATE_PATH . '/' . $endpoint_package, $temp_location . $endpoint_package))) {
                        ...
                        if ($echotxt) {
                            echo format_txt(_("Not able to connect to repository. Using local Provisioner.net Package."), "error");
                        }
                    } else {
                    ...
                    foreach ($out as $data) {
                        ...
                        if (!$local) {
                            ...
                            if (!$result) {
                                ...
                                if ($echotxt) {
                                    echo format_txt(_("Not able to connect to repository. Using local brand [%_brandname_%] file instead."), "error", array("%_brandname_%" => $data['name']));
                                }
                            }
                        }
                ...
                if ($echotxt) {
                    echo format_txt(_("The Remote Server Is Currently Syncing With the Master Server, Please try again later"), "error");
                }
            }
        } else {
            $o = getcwd();
                    ...
                    foreach ($temp['data']['brands']['family_list'] as $family_list) {
                        ...
                        foreach ($family_line_xml['data']['model_list'] as $model_list) {
                            ...
                            if (!$this->sync_model($brand_id . $family_line_xml['data']['id'] . $model_list['id'])) {
                                ...
                                if ($echotxt) {
                                    echo format_txt(_("System Error in Sync Model [%_name_%] Function, Load Failure!"), "error", array("%_name_%" => $model_list['model']));
                                }
                            }
                        }
    ...
    function remove_firmware($id)
    {
        ...
        foreach ($file_list as $file) {
            ...
            if (!file_exists($this->configmod->get('config_location') . $file)) {
                continue;
            }
            if (!is_file($this->configmod->get('config_location') . $file)) {
                continue;
            }
